...
class VideoProcessingApp:
    ...
    def predictImg(self):
        ...
        if self.data["status"] == 200:
            ...
            for i in list_input:
                text += i
                text += "，"
            text += "请你作为一名环保专家，针对这些垃圾分类情况，帮我生成一些实质性的建议，包括如何正确处理这些垃圾、环保影响以及提高垃圾分类意识等。只需回答我要的结果。"
            messages = [
    ...
    def predictVideo(self):
        ...
        self.data.update({
            ...
            "thinkMode": request.args.get('thinkMode') == 'true' or request.args.get('thinkMode') == True
        })
